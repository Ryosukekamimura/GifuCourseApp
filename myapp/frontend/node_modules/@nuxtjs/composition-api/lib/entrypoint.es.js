...
const isFullStatic = "<%= options.isFullStatic %>".includes("options") ? false : false;
...
const getHeadOptions = (options) => {
  const head = function() {
    ...
    return defu2({}, ...computedHead.reverse(), optionHead);
  };
  return {head};
};
const useMeta = (init) => {
  ...
  if (process.client) {
    ...
    watch(key, (key2) => {
      ...
      if (!process.static)
        ...
        fetch(`<%= options.publicPath %>${key2}.json`).then((response) => {
          if (!response.ok)
...
export { defineComponent, defineNuxtMiddleware, defineNuxtModule, defineNuxtPlugin, defineNuxtServerMiddleware, globalPlugin, onGlobalSetup, reqRef, reqSsrRef, setMetaPlugin, setSSRContext, shallowSsrRef, ssrPromise, ssrRef, useAsync, useContext, useFetch, useMeta, useStatic, withContext };
